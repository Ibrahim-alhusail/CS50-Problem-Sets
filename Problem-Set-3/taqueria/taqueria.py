...
until the user inputs control-d (which is a common way of ending one’s input to a program for mac and linux) control-z for windows.
...
def main():
    ...
    while True:
        try:
            ...
            total += take_order(item.title(), total)
        except EOFError as e:
            print(f"Total: ${total}")
            exit()


def take_order(item, total):
    menu = {
        "Baja Taco": 4.25,
        "Burrito": 7.50,
        "Bowl": 8.50,
